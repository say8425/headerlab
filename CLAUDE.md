...
scripts/         not shipped: image generators, store packing/submission/probes · lib/
                 holds their pure halves (cws, crx, amo, png, popup-shots)
...
for the hand-written `PURE_FILES` list elsewhere. **A new pure file outside those two
directories is unguarded until you add it to `PURE_FILES` by name** — and so is any pure module
...
- README screenshots are generated by `pnpm screenshots`, never cropped by hand.
...
| File | Covers |
| --- | --- |
| `toolchain.md` | pnpm, lockfile, install scripts, oxlint, oxfmt, TypeScript, Tailwind sources |
| `ci-release.md` | CI jobs, action pinning, release-please, tags, changelogs, npm publishing |
| `chrome-web-store.md` | listing, verified CRX signing, store API, store assets |
| `firefox-amo.md` | AMO submission, credentials, throttling, listing, sources archive |
| `extension-runtime.md` | platform traps (DNR, permissions, WXT, migrations, Firefox), runtime gaps |
| `popup-ui.md` | layout rules, one-line text, styling traps, measuring the rail |
| `testing.md` | suites, e2e, colour coverage, mutation and race discipline, CLI tests |
