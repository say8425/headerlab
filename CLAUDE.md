...
Observed waits, all from one afternoon: 56 seconds after three quick writes, 1571 before the
privacy policy would go, 3433 before a fourth screenshot, 933 for the release's
version-create. Read it as one bucket per account rather than one per endpoint, **do not fill
...
measured here: wait out the seconds the `429` names, then `workflow_dispatch` `amo-submit.yml`
against the same tag with `ref` empty. Nothing is duplicated, because the refused call is the
one that would have created the version — the re-run repeats the upload and succeeds. The privacy policy needed a second
