...
test('an unreachable bridge leaves the rail exactly where a live one does', async () => {
  ...
  // cap went 127 -> 119 -> 127 inside one branch, and 127 -> 108 when the rows
  ...
  expect(idleMeasurement.boxes['[data-testid="site-list"]']![3]).toEqual(108);
  expect(liveMeasurement.boxes['[data-testid="site-list"]']![3]).toEqual(108);
