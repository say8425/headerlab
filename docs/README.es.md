...
Doce comandos en total, y cada respuesta es un único objeto JSON en stdout con el código de
salida detrás. **El puente está apagado hasta que una persona lo enciende** — va montado
sobre `nativeMessaging` como permiso opcional, detrás del propio diálogo de consentimiento
de Chrome, y la CLI no puede ni encenderlo ni conceder un sitio: Chrome exige un gesto del
usuario para ambas cosas. Nada sale de la máquina; los tres procesos hablan por un socket
de dominio Unix en un directorio por usuario, nunca por un socket de red.

Encenderlo son tres pasos — pulsar **Enable** en la fila del puente del popup, ejecutar
`headerlab bridge install --extension-id <id>`, y el popup pasa a leer **Bridge live**.

**[→ `docs/agent-bridge.md`](agent-bridge.md)** tiene el diseño: el diagrama de procesos y
por qué la dirección es de un solo sentido, la tabla completa de comandos, las cinco
afirmaciones que conviene no malinterpretar, y qué hacer cuando una actualización deja
huérfano al lanzador. La referencia de comandos en sí vive en
