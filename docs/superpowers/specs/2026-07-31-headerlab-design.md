...
**`globalPause` 가 켜져 있으면 `dynamic` 과 `session` 이 모두 빈 배열이다.** 진단은 그대로
계산해서 내보낸다 — 일시정지 중에도 사용자가 설정의 문제를 볼 수 있어야 하고, 재개했을 때
비로소 경고가 나타나면 놀라게 된다.
...
`condition.requestDomains` 를 쓰면 이 문제가 크게 줄어든다. 도메인 목록은 오리진 패턴으로
기계적으로 변환되므로 `permissions.contains()` 로 검사할 수 있다.

- **기본(`structured`)** — 도메인 목록 + 경로 패턴. `requestDomains` 로 컴파일.
  권한 감지가 신뢰할 수 있다(§5.4 의 정규화 규칙 적용 시).
...
### 4.3.1 `pathPattern` → `urlFilter` 변환

`urlFilter` 는 **직렬화된 URL 전체에 대한 부분 문자열 매칭**이지 경로 매칭이 아니다.
`/v2/` 를 그대로 넣으면 `https://x.com/a?q=/v2/` 처럼 쿼리 문자열에도 걸린다.

따라서 경로 패턴은 그대로 넣지 않고 앵커를 붙여 변환한다. 사용자가 `/v2/*` 를 입력하면
컴파일러는 도메인 앵커와 결합해 `||<domain>^*/v2/` 형태를 만든다. 도메인이 여러 개면
경로 패턴은 도메인당 하나씩 룰을 만드는 대신 `requestDomains` 로 도메인을 처리하고
`urlFilter` 에는 경로 조각만 앵커 없이 두되, **UI 가 미리보기로 실제 매칭 대상을 보여준다**.

이 변환은 `compile/conditions.ts` 안의 순수 함수이고, 앵커 조합별 테이블 테스트 대상이다.
`urlFilter` 에 이스케이프가 없으므로(§11.3) 경로에 `*` `|` `^` 가 포함되면 정규식 모드로
전환하도록 UI 가 안내한다.
...
`permissions.contains()` 는 **"내가 이 패턴 이상을 가지고 있는가"** 를 묻는 부분집합 검사다.
여기에 함정이 있다.

`requestDomains` 에는 스킴이 없으므로 도메인에서 유도한 패턴은 자연히 `*://*.D/*` 가 된다.
그런데 사용자가 `https://api.example.com/*` 만 허용한 상태에서 `*://*.api.example.com/*` 를
검사하면 **false 가 나온다** — `*://` 가 `https://` 보다 넓고 `*.` 서브도메인 와일드카드도
더 넓기 때문이다. 실제로는 잘 동작하는 설정인데 "권한 필요" 배지가 뜬다.

배지가 한 번이라도 거짓 양성을 내면 사용자는 배지를 무시하기 시작하고, 그러면 이 기능의
존재 이유가 사라진다. 그래서 감사는 **좁은 것에서 넓은 것 순으로 후보 패턴을 검사하고,
하나라도 통과하면 부여된 것으로 본다.**

도메인 `D` 에 대한 후보 순서:

```
1. https://D/*          정확한 호스트, https
2. https://*.D/*        서브도메인 포함, https
3. *://D/*              정확한 호스트, 모든 스킴
4. *://*.D/*            가장 넓음
```

하나라도 `contains()` 가 true 면 통과로 처리한다. 전부 false 일 때만 `permission-missing`
진단을 낸다.

**요청할 때는 반대로 넓은 것을 요청한다** — `*://*.D/*`. 감사는 관대하게, 요청은 넉넉하게.
그래야 사용자가 한 번 허용한 뒤 스킴이 바뀌거나 서브도메인이 추가돼도 다시 묻지 않는다.

이 규칙은 §4.3 이 구조화 필터를 채택한 이유의 전제이므로, 구현 초기에 실제 확장에서
`contains()` 동작을 확인해 규칙이 맞는지 검증한다(§11.7).
...
**테마 적용 위치는 `document.documentElement` 로 고정한다.** shadcn 이 기본 생성하는 다크
배리언트 `&:is(.dark *)` 는 `.dark` 클래스를 단 요소 **자신에게는 적용되지 않고** 자손에만
적용된다. 팝업 안쪽 컨테이너에 `.dark` 를 달면 그 컨테이너 자신의 배경이 빠져 어긋난다.
루트 엘리먼트에 달면 shadcn 기본값을 그대로 쓸 수 있으므로 배리언트를 고칠 필요가 없다.
(내부 컨테이너 방식을 택해야 할 일이 생기면 `&:where(.dark, .dark *)` 로 바꿔야 한다.)

**첫 페인트에 테마가 번쩍이면 안 된다.** `theme` 은 `local:state` 에 있고 `storage.getItem`
은 비동기다. 그대로 두면 팝업이 잘못된 테마로 그려졌다가 고쳐진다. 하루에 수십 번 여는
표면에서 이 깜빡임은 정확히 잘못된 첫인상이고, 이 프로젝트가 존재하는 이유와 정면으로
어긋난다.

따라서 문서 파싱 시점에 **동기적으로** `prefers-color-scheme` 을 읽어 루트에 클래스를 달고,
저장된 값이 `system` 이 아니면서 다르게 확정된 경우에만 비동기 로드 후 교체한다. 대부분의
사용자는 `system` 이므로 교체 자체가 일어나지 않는다.
...
7. **`permissions.contains()` 의 패턴 부분집합 판정 정확한 동작.** §5.4 의 후보 순서 규칙은
   "넓은 패턴 검사는 좁은 부여에 대해 false 를 낸다"는 전제 위에 서 있다. 합리적인 추론이고
   부분집합 의미론에 부합하지만 실제 확장에서 확인한 것은 아니다. **구현 첫 주에 검증한다** —
   틀렸다면 후보 목록을 줄일 수 있고, 맞다면 그대로 간다. 어느 쪽이든 배지 신뢰도가 걸려
   있으므로 추측으로 두지 않는다.

8. **`activeTab` 이 DNR 의 initiator 호스트 권한 검사에 계산되는지.** Chromium 의
   `CanExtensionAccessURL` 이 `tab_id` 를 인자로 받으므로 아닐 것이라 단정할 수 없다.
   현재 설계는 이에 의존하지 않으므로 막히는 것은 없다. 다만 계산된다면 §5.3 의 도메인별
   허용 경로가 훨씬 편해진다 — 사용자가 보고 있는 페이지 오리진을 따로 허용받지 않아도
   되기 때문이다. 여유가 생기면 확인할 가치가 있다.
