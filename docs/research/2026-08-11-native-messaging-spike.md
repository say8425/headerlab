...
**이 스파이크 이전에 이 저장소가 갖고 있던 반대 서술은 틀렸다.** "포트를 여는 것은 더 이상
타이머를 리셋하지 않는다"는 문장은 **일반 runtime 포트**에 대한 것이고, 네이티브 메시징에
확대 적용한 것이 오류였다. 같은 문서의 다른 줄 — 네이티브 메시징은 두 타이머를 모두
취소하는 강한 keep-alive 를 제공한다 — 이 맞았다. 근거로 인용됐던 이슈 두 건도 조사 단계에서
반박됐다: 하나는 Linux/Chromium 이고 보고자의 추측이었으며, 다른 하나는 Chrome 105/110
이전의 증거다.
