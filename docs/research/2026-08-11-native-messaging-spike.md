...
## `unlink()` 동시 경합: 텍스트북 POSIX 원자성이 이 파일시스템에서는 깨진다

`packages/host/lib/socket.mjs` 의 `removeRegistryEntry`/`removeStaleSocket` 이 쓰는
unlink-if-exists 패턴(ENOENT 를 성공으로 삼는 것)을 교차 프로세스 경합에서도 검증하려던
시도. 두 개의 별도 `node` 자식 프로세스가 같은 경로에 대해 진짜 `unlinkSync` 를
경합시켰다 — 수정 있음/없음 양쪽에 대해, 이 환경의 샌드박스 안과 밖 양쪽에서
뮤테이션 검증.

**측정값: ENOENT 캐치를 제거해도 시행의 약 15–20% 에서 두 프로세스 모두
`unlinkSync` 가 성공을 보고했다.** 즉 두 프로세스가 충분히 가깝게 경합하면 이
파일시스템의 `unlink()` 는 정확히 한쪽에게만 ENOENT 를 주지 않는다 — 텍스트북
POSIX 원자성 가정과 다르다.

**결론: "두 프로세스 모두 던지지 않아야 한다" 형태의 테스트는 되돌린 수정에서도
운 좋게 통과할 수 있다.** "운으로 통과하는 타이밍 의존 테스트는 없느니만 못하다"는
경우라, 그런 테스트는 커밋하지 않았다. 대신 실제로 성립하고 경합 재현 없이도
증명되는 것: ENOENT 를 잡는 것은 "존재하면 지운다" 연산에서 대상이 없는 이유(한
번도 생성되지 않음, 같은 프로세스의 이전 호출이 지움, 동시 프로세스가 지움)와
무관하게 무조건 옳다 — 이 보장은 특정 인터리빙을 재현하는 것이 아니라
`unlinkIfExists` 자체를 들여다보는 것만으로 성립한다.
