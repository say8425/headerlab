...
HeaderLab 은 AI 코딩 에이전트로 조작할 수 있습니다. 세 조각이 층을 이룹니다. 사람이 손으로
쓸 수도 있는 CLI, 에이전트에게 그 CLI 사용법을 가르치는 스킬, 그리고 둘 중 어느 쪽이든 실행
중인 확장까지 연결해 주는 브리지입니다. 셋 다 기본으로 켜져 있지 않고, 스스로 켜지지도
않습니다. 그 이유는 이 절의 마지막 문단에 있습니다.
...
터미널에서 확장을 조작하는 `headerlab` 명령이 PATH 에 놓입니다.
[에이전트 브리지](#에이전트-브리지)를 보세요. 패키지에 런타임 의존성이 하나도 없어서
클론에서 설치 없이 바로 실행할 수도 있습니다. `node packages/headerlab/bin/headerlab.mjs`
입니다. 위의 한 줄이 사람이 쓰는 방법이고 클론은 기여자가 하는 일이라, 순서는 일부러 이렇게
두었습니다.
...
스킬은 자기 내용이 모델에 닿기 전에 `command -v headerlab` 를 실행합니다. CLI 가 없다는
사실을 작업 도중에 알게 되는 대신 처음부터 알게 하려는 것입니다. **브리지를 켜기 전까지는
`bridge-off` 를 보고합니다.** CLI 를 전역 설치해 두는 것이 전제 조건은 아닙니다. 플러그인이
`packages/headerlab` 를 가리키는 shim 을 자체적으로 들고 있습니다. `npm i -g headerlab` 를
같이 해도 충돌하지 않습니다. PATH 가 전역 사본을 먼저 찾습니다.
...
첫 줄과 마지막 줄은 읽기입니다. `status`, `site ls`, `rule ls`, `state get` 이 아무것도 쓰지
않고 답합니다. 가운데 셋은 쓰기입니다. 여기서 하나는 미리 알아두는 편이 좋습니다. 사이트를
추가하면 룰의 적용 범위가 정해질 뿐, 그 사이트에 대한 접근 권한이 생기지는 않습니다. 팝업에서
Grant 를 누르기 전까지 그 사이트는 대기 상태로 남습니다. 스킬은 이미 적용된 것처럼 넘어가지
말고 그 사실을 말하도록 지시받아 있습니다.
...
브리지는 사람이 팝업에서 스위치를 켜기 전까지 꺼져 있습니다. CLI 는 사이트 접근 권한을 줄
수도, 브리지를 켤 수도 없습니다. Chrome 이 둘 다 사용자 제스처로만 받기 때문입니다. 기기
밖으로 나가는 것은 없습니다. CLI 와 호스트와 확장은 사용자별 디렉터리에 있는 유닉스 도메인
소켓에서 만나고, 네트워크 소켓은 쓰지 않습니다.

브리지에 관한 모든 것은 [`docs/agent-bridge.ko.md`](agent-bridge.ko.md) 에 있습니다.
프로토콜, 명령, 종료 코드, 켜는 법, 그리고 오해하면 안 되는 다섯 가지.

## 무엇을 하는가

- **요청** 쪽이든 **응답** 쪽이든, 어떤 헤더든 **설정·추가·삭제**합니다. 요청 헤더의
  `append` 는 Chrome 이 21개 허용목록으로 제한하며, 그 밖에 있는 룰은 HeaderLab 이 짚어
  줍니다. 들리는 것보다 중요한 이야기입니다. Chrome 은 룰셋을 룰 단위가 아니라 통째로
  거절하기 때문에, 그런 룰 하나가 나머지 전부를 함께 멈춥니다. 조용히 넘어가지도 않습니다.
  팝업이 등록 실패를 표시합니다.
- **사이트 단위로 범위를 정합니다.** 사이트는 호스트로 매칭합니다. 포트나 경로를 넣으면
  떨어져 나갑니다. 저장된 값이 곧 동작하는 값이라, 레일에 보이는 것이 실제로 회선에
  나갑니다.
- **모든 사이트에 적용**은 빈 목록이 아니라 명시적인 모드입니다. `<all_urls>` 가 필요하지만
  스위치 자체는 그것을 요청하지 않습니다. 옆의 Grant 버튼이 요청합니다.
- **요청 타입으로 거릅니다.** Chrome 의 리소스 타입 여덟 개를 하나씩 체크합니다.
  `main_frame` 은 기본으로 켜져 있습니다. DNR 의 기본값이 그것을 조용히 빼기 때문입니다.
- 스위치 하나로 **전체를 일시정지합니다.** 툴바 아이콘도 회색으로 바뀝니다. 서비스 워커가
  깨어날 때 다시 적용됩니다.
- **OS 테마를 따릅니다.** 라이트든 다크든 첫 페인트 전에 정해집니다.
...
<sub>Chrome 에 로드한 실제 프로덕션 빌드에서 촬영했습니다. 매니페스트만 손댔습니다. 예시
호스트 두 개를 미리 허용해 두어야 네이티브 권한 대화상자 없이 허용된 상태를 찍을 수 있기
때문입니다.</sub>

## 신뢰 원칙

- **설치 시점에는 호스트 권한이 없습니다.** 매니페스트의 `permissions` 는 `storage` 와
  `declarativeNetRequestWithHostAccess`, 정확히 이 둘뿐입니다.
  `optional_host_permissions: ["<all_urls>"]` 도 선언하지만, 이 선언 자체로 부여되는 것은
  없습니다. Chrome 은 확장이 선언한 적 없는 오리진을 요청하면 거부합니다. 그러니까 저 줄은
  런타임 Grant 버튼을 성립시키는 것이지, 불필요하게 만드는 것이 아닙니다. 사이트 접근 권한은
  사용자가 호스트마다 런타임에 부여하고, Chrome 에서 언제든 회수할 수 있습니다.
- **네트워크 호출이 없습니다.** 분석도, 텔레메트리도, 원격 설정도, 업데이트 핑도 없습니다.
  배포되는 번들은 네트워크 프리미티브를 *호출*하지 않습니다. 믿는 대신 직접 확인할 수
  있습니다:
  ...
  아무것도 나오지 않습니다. 이 패턴이 호출 형태와 생성자 형태만 잡는 것은 의도한 것입니다.
  대소문자를 무시하고 저 단어들을 그냥 찾으면 번들에서 열여섯 번 걸립니다. 전부 호출이 아니라
  문자열이거나 식별자입니다. React DOM 의 `prefetchDNS`, `fetchPriority`, `dns-prefetch`,
  그리고 리터럴 `"xmlhttprequest"` 와 `"websocket"` 입니다. 뒤의 둘은
  declarativeNetRequest 의 리소스 타입 이름이고, 들어온 경로는 서로 다릅니다.
  `xmlhttprequest` 는 팝업이 체크박스로 제공하는 여덟 개 중 하나입니다(거기서는 `xhr` 로
  표시됩니다). `websocket` 은 저장된 상태를 검증하는 열다섯 개짜리 리소스 타입 enum 의
  원소일 뿐입니다. 나중에 발견했을 때 들킨 거짓말이 아니라 예상된 것으로 읽히도록 여기 적어
  둡니다.
- **콘텐츠 스크립트가 없습니다.** 어떤 페이지에도 아무것도 주입하지 않습니다. 헤더는
  Chrome 의 `declarativeNetRequest` 엔진이 바꿉니다. 이 엔진은 요청 내용을 확장에 넘기지
  않습니다.
- **외부 리소스가 없습니다.** CDN 도, 웹폰트도, 원격 이미지도 없습니다.
- **조용한 실패가 없습니다.** 룰이 나가지 못하게 하는 것은 화면에 말합니다. 없는 권한, 쓸 수
  없는 호스트명, Chrome 이 거절할 헤더 이름이 그렇습니다. 적용되지 않는 룰은 언제나 이유를
  말합니다.

## 한계

자세한 건 [MDN 브라우저 호환성 데이터](https://github.com/mdn/browser-compat-data)를 참고하세요.
...
**정확성은 전부 `chrome.*` 를 절대 import 하지 않는 순수 계층에 있습니다.** `compile()` 은
애플리케이션 상태 전체를 declarativeNetRequest 룰과 진단 목록으로 바꿉니다. 팝업도 같은
상태에 같은 함수를 돌립니다. 그래서 화면이 말하는 것과 브라우저가 들은 것이 어긋날 수
없습니다.

**reconcile 루프는 하나뿐입니다.** 저장소 변경, 워커 시작, 권한 부여와 회수까지 모든 트리거가
`lib/sync/ruleSync.ts` 의 `reconcile()` 로 모입니다. `reconcile()` 은 처음부터 다시
컴파일해서 룰셋을 통째로 갈아끼웁니다. 멱등이고, 상태가 아래로 흘러내릴 두 번째 경로는
없습니다.

이 구조는 고른 것이 아니라 강제된 것입니다. `@webext-core/fake-browser` 는
`declarativeNetRequest` 와 `permissions.*` 를 예외를 던지는 스텁으로 구현하므로, 브라우저를
흉내 내는 테스트가 불가능합니다. 로직이 브라우저와 무관해지도록 만드는 것이 그에 대한
답입니다.
...
지정합니다. `corepack enable` 하나면 그 버전이 오고, 따로 설치할 것은 없습니다.
`package-lock.json` 은 없습니다. CI 가 `--frozen-lockfile` 로 설치하는 락파일은
`pnpm-lock.yaml` 입니다.

**`pnpm exec vitest run` 을 맨손으로 돌리지 말고 `pnpm test` 를 쓰세요.** 여러 스위트가
*빌드된* 산출물에 대해 단언하는데, 맨손 도구는 빌드를 하지 않습니다. 낡은 산출물 때문에
...
씁니다. 그게 이 명령들이 하는 일입니다. 다만 한 번 돌리면 `git status` 에 변경이 남으니, UI
가 실제로 바뀌었을 때만 커밋하세요.

**e2e 빌드에는 배포 빌드에 없는 호스트 권한이 들어 있습니다. 이 페이지 첫머리의 주장을
생각하면 소리 내어 말할 값어치가 있습니다.** `pnpm test:e2e` 는 프로덕션 디렉터리 옆에
`.output/chrome-mv3-e2e` 와 `.output/chrome-mv3-bridge-e2e` 를 만듭니다. 앞의 것은
`http://127.0.0.1/*` 를 선언합니다(`wxt.config.ts`). Playwright 가 클릭할 수 없는 런타임
프롬프트 없이 로컬 에코 서버를 구동하기 위해서입니다. 뒤의 것은 `nativeMessaging` 을 곧바로
부여합니다. 둘 다 프로덕션에 닿지 않는다는 것은 `tests/unit/manifest.test.ts` 가 단언합니다.
e2e 스위트를 돌려도 `.output/chrome-mv3` 는 건드리지 않으니, 새 프로덕션 빌드는 `pnpm build`
로 만드세요.

나머지는 `../CLAUDE.md` 에 있습니다. `lint` 가 왜 `wxt prepare` 를 체인하는지,
`postinstall` 이 왜 아예 안 돌 수 있는지, oxfmt 가 무엇을 포맷하고 무엇을 하지 않는지,
그리고 이미 누군가의 시간을 태운 플랫폼 함정들.

## 테스트

세 층으로 되어 있습니다. 브라우저 없이 도는 순수 로직, 손으로 심은 스파이로 구동하는 어댑터,
그리고 실제로 로드된 확장을 상대하는 종단간 테스트입니다. e2e 중 둘은 로컬 에코 서버를 통해
실제 요청을 회선에 올리고 헤더를 되읽습니다. 이 저장소에서 가장 강한 증거입니다. 브리지도
자기 몫의 테스트를 갖고 있습니다. 그중 하나는 실제 `headerlab site add` 를 실제로 설치된
호스트와 소켓을 거쳐 실제 저장소까지 밀어 넣습니다.

`packages/headerlab` 은 자체 스위트를 들고 있고, vitest 가 아니라 Node 내장 테스트 러너로
돌립니다. 그 패키지는 의존성이 없고 앞으로도 가져서는 안 되기 때문입니다.
`vitest.config.ts` 의 glob 은 거기에 닿지 못합니다. 그것이 자체 CI 잡을 갖는 이유입니다.
한동안은 실행되지 않은 채 머지되고 있었습니다. 아무것도 돌리지 않는 스위트는 없는 것보다
나쁩니다. 성공을 보고하기 때문입니다.
