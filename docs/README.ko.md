...
- **네트워크 호출 없음.** 분석, 텔레메트리, 원격 설정, 업데이트 핑 어느 것도 없습니다.
  ...
  무시하고 저 단어들을 그냥 찾으면 번들에서 열네 번쯤 걸리는데, 전부 호출이 아니라 문자열
...
명령은 모두 열두 개이고, 모든 응답은 stdout 의 JSON 객체 하나에 종료 코드가 뒤따르는
형태입니다. **사람이 켜기 전까지 브리지는 꺼져 있습니다** — `nativeMessaging` 을 선택적
권한으로 타고, Chrome 자신의 동의 대화상자 뒤에 있습니다. 그리고 CLI 는 브리지를 켤 수도,
사이트 권한을 줄 수도 없습니다. Chrome 이 둘 다 사용자 제스처를 요구하기 때문입니다.
기기 밖으로 나가는 것은 없습니다 — 세 프로세스는 사용자별 디렉터리의 유닉스 도메인 소켓으로만
이야기하며, 네트워크 소켓은 쓰지 않습니다.

켜는 것은 세 단계입니다 — 팝업의 브리지 행에서 **Enable** 을 누르고,
`headerlab bridge install --extension-id <id>` 를 실행하면, 팝업이 **Bridge live** 를
읽습니다.

**[→ `docs/agent-bridge.md`](agent-bridge.md)** 에 설계가 있습니다: 프로세스 다이어그램과
방향이 왜 한쪽뿐인지, 전체 명령 표, 오해하면 안 되는 다섯 가지, 그리고 업그레이드가 런처를
고아로 만들었을 때 할 일. 명령 레퍼런스 자체는
