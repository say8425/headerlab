...
- **CLI はブリッジも有効にできません。** `chrome.permissions.request()` は解決するのに
  ...
  生まれることもありません: `bridge install` はマニフェストを書くだけで、人が
  スイッチを入れるまでは何も接続しません。
...
設計自身の §2/§3 が名前を挙げたもののうち 2 つは作りません。`headerlab diagnostics`
は、`status` が同じペイロードを運んでおり、1 つのクエリに 2 つ目の名前を与えるのは機能では
ないからです。そして `state snapshots`/`state restore <id>` が読み戻すはずだった、raw な
書き込みの前のスナップショットは、2026-08-22 に作らないことに
決めました。`state set` はスキーマ検証を行い
